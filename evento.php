   ...
   if ($nome && $data && $local && $tema_id && $objetivo_id && $buffet_id && $faixa_etaria_id && $status_social_id && $status_do_evento_id && $escolaridades_id) {
    $nomeImagem = null;
        ...
        if ($stmt->execute()) {
            ...
            // Insere a associação na tabela intermediária evento_buffet
            $stmt_assoc = $conn->prepare("INSERT INTO evento_buffet (evento_id, buffet_id) VALUES (?, ?)");
            if ($stmt_assoc === false) {
                die('Erro ao preparar a consulta para associação: ' . $conn->error); // Exibe erro detalhado
            }
            $stmt_assoc->bind_param("ii", $evento_id, $buffet_id);
            
            if ($stmt_assoc->execute()) {
                $cadastroSucesso = true; // Se ambas inserções foram bem-sucedidas
            } else {
                echo "Erro ao associar buffet ao evento: " . $stmt_assoc->error;
            }


// Insere a associação na tabela intermediária evento_objetivo
...
// Executa a inserção
if ($stmt_assoc->execute()) {
    $cadastroSucesso = true; // Se a inserção foi bem-sucedida
} else {
    echo "Erro ao associar objetivo ao evento: " . $stmt_assoc->error;
}


// Insere a associação na tabela intermediária evento_tema
...
if ($stmt_assoc->execute()) {
    ...
    //echo "Associação criada com sucesso entre o evento e o tema!";
} else {
    echo "Erro ao associar evento ao tema: " . $stmt_assoc->error;
}
